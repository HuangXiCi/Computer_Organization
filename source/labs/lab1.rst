...
我们的框架中给出了 vscode gdb 调试 qemu 的方法，并给出了launch.json，感兴趣可以自行研究。

我们通过在代码框架目录下终端输入 ``make gdb`` 启动 qemu 等待调试

.. figure:: ../picture/lab1/start_gdb.png
   :alt: start_gdb
   :align: center

我们可以先在任意一行打断点，然后在左侧 Run and Debug 中点击上方的绿色播放键开启 gdb 调试。
此时gdb就会启动，qemu会运行停留在打断点的那行。

.. figure:: ../picture/lab1/learn_gdb.png
   :alt: learn_gdb
   :align: center

开启调试之后，左侧会出现调试信息，可以观察寄存器的值，这些寄存器都是使用的 ABI 名称。
