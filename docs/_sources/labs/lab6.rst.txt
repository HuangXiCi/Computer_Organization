...
处理器本身并不会直接理解这些设备的物理工作方式，它通常只是通过某种总线读写外设内部的寄存器。
对软件来说，外设寄存器看起来就像一段特殊的内存地址。
这种方式称为 **内存映射 I/O**，也就是 Memory Mapped I/O，简称 ``MMIO``。
...
由于实验课程的时间限制，本次实验主要完成一个简单的 UART 发送模块，并将其接入处理器的数据访存通路。
通过本次实验，你需要理解：

- 处理器如何通过 load/store 指令访问外设；
- 什么是 MMIO，为什么外设可以被映射到地址空间中；
- 地址解码器如何根据访问地址选择 SRAM 或 UART；
- UART 串口发送的基本时序；
- 软件如何通过轮询状态寄存器向 UART 输出字符。
...
- 发送数据寄存器：软件向该寄存器写入一个字符，UART 就开始发送；
- 状态寄存器：软件读取该寄存器，判断 UART 当前是否忙；
- 接收数据寄存器：如果实现接收功能，软件可以从这里读出收到的字符。

这些寄存器并不一定是真正的 SRAM 单元。
它们可以是外设模块内部的一些触发器、状态机信号或组合逻辑输出。
但是对处理器来说，只要它们被映射到了某个地址，处理器就可以通过普通的 ``lw``、 ``sw``、 ``lb``、 ``sb`` 等访存指令来访问它们。
...
``0x1000_0000`` 这个地址不是普通内存，而是 UART 的发送数据寄存器。
当软件执行：
...
处理器会发起一次 store 操作。
硬件地址解码器发现访问地址位于 UART 区域，于是不会把写使能送给数据 SRAM，而是送给 UART 外设。
UART 收到写入的数据 ``'A'`` 后，就把这个字符转换成串口波形发送出去。

.. raw:: html

   <div class="admonition myhint">
     <p class="admonition-title">MMIO 和普通内存的区别</p>
     <p>普通内存的读写目标是 SRAM 或 RAM 单元，读写结果通常只是改变存储器中的数据。
     MMIO 的读写目标是外设寄存器，一次读写可能会触发硬件行为。
...
读数据时，地址解码器也需要根据高16位地址，从这三个设备中选择出需要的那个。
...
- ``UART_TX_DATA``：地址 ``0x1000_0000``，写入低 8 位数据后启动发送；
- ``UART_RX_DATA``：地址 ``0x1000_0004``，读取 UART 接收数据；

如何发送数据：
当CPU读取 UART_TX_DATA 寄存器的值为0，则 UART 发送寄存器空闲，可以写入要发送的数据，
如下C代码示意， ``char inb(addr)`` 函数相当于lb addr地址读取一个字节数据。
``void outb(addr)`` 函数则相当于sb addr地址写入一个字节的数据。
当 UART_TX_DATA 不为 0 时， UART 发送寄存器忙，一直循环读取，直到为 0 ，则可以写入发送数据。
...
如何读取数据：
当CPU读取 UART_RX_DATA 寄存器值为0，则 UART 没有接收到数据，则一直循环读取，直到不为0，
则代表读取到接收数据了。如下C代码示意。
...
要发送数据，则CPU往 UART_TX_DATA 中写非零值之后，则开始启动发送数据状态机，
状态机将待发送的数据送入移位寄存器中，等待移位发送数据，同时会清空 UART_TX_DATA 寄存器的值，
因为此时待发送的数据已经缓存到移位寄存器中，可以接收新的发送数据了。

状分频计数器开始计数，以确保波特率满足规定。状态机会依次经历起始位（tx_pad 拉低），数据位，停止位（tx_pad 拉高），最后回到IDLE状态，
随时准备下一次数据发送。
...
直到整个数据帧接收完毕，将接收数据的移位寄存器值写入 UART_RX_DATA ，当 CPU 发现值不为 0 ，
则接收到了数据， UART_RX_DATA 会在每次 CPU 读该寄存器时清空值为 0 ，防止 CPU 误取到已经取走的数据。
...
.. raw:: html
   ...
   <div class="admonition mytodo">
     ...
     <p>我们在代码框架中给大家准备了testbench.sv用于测试你的处理器现在功能是否正常，通过所有测试。</p>
   </div>
